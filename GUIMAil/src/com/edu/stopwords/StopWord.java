...
public class StopWord {
	...
	/**
	 * 
	 * @param arrLetter Mang danh sach cac tu can phan loai
	 * @param isStopWord Chon dau ra mong muon. True la cac tu StopWord, False cac tu hop le
	 * @return tra ve danh sach cac tu sau khi da duoc loai bi
	 * @author cuongnv
	 */
	public ArrayList<String> letterStopWords(ArrayList<String> arrLetter,boolean isStopWord){
		ArrayList<String> arr = new ArrayList<>();
		for(int i=0;i<arrLetter.size();i++){
			if(arrLetter.get(i).equalsIgnoreCase(" ")||arrLetter.get(i).equalsIgnoreCase("")){
				continue;
			}
			if(isStopWord){
				if(map.get(arrLetter.get(i))== null){
					continue;
				}
			}else{
				if(map.get(arrLetter.get(i)) != null){
			...
			try{
				Integer.parseInt(arrLetter.get(i));
				continue;
			}catch (NumberFormatException e){
			}
			arr.add(arrLetter.get(i));
		}
		return arr;
	}
	/**
	 * Tim trong van ban cac tu muon tim
	 * @param keywords : Mang cac tu khoa dung lam khoa tim kiem
	 * @param text : Van ban tim kiem
	 * @return Tra ve mang cac tu tim thay trong van ban
	 * @author cuongnv
	 */
	public static ArrayList<String> letterKeyWords(Set<String> keywords,String text){
		ArrayList<String> arr = new ArrayList<>();
		text = text.toLowerCase();
		text = text.trim();
		text = text.replaceAll("\b", " ");
		...
		text = text.replaceAll("\\)", " ");
		String [] letter = text.split(" ");
		int leng = letter.length;
		for(int i = 0;i < leng; i++){
			if(keywords.contains(letter[i])){
				if(arr.indexOf(letter[i])==-1||arr.size() == 0){
					arr.add(letter[i]);
				}
			}
		}
	...
	/*public static void main(String []args){
		 ...
		 ArrayList<String> t = tw.letterStopWords(text, true);
